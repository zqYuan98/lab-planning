...
// Libraries change far less often than the app, so they get their own content-hashed chunk that
// browsers keep across releases. Packages reached only through lazy imports stay lazy.
const lazyPackages = new Set(['dingtalk-jsapi', 'promise-polyfill', 'docx-preview', 'jszip'])
function vendorChunk(id: string) {
  const name = id.match(/node_modules[\\/]((?:@[^\\/]+[\\/])?[^\\/]+)/)?.[1]
  if (!name || lazyPackages.has(name)) return undefined
  return 'vendor'
}
export default defineConfig({
  ...
  build: { manifest: true, assetsInlineLimit: 0, rollupOptions: { output: { manualChunks: vendorChunk } } },
