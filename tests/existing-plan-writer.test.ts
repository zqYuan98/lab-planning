...
test('imported omissions stay editable while ordinary creation remains strict', t => {
  ...
  const updatedWeek = f.domain.updateWeeklyRecord(f.member, imported.weekly.id, { version: imported.weekly.version, commitment: '', nextAction: '下一步核对' })
  const updatedTask = f.domain.updateTask(f.member, task.id, { reason: '测试场景确认承诺调整', version: task.version, dueDate: '', description: task.description, title: '调整名称' })
  assert.equal(updatedPlan.dueDate, '')
  assert.equal(updatedWeek.actualOutcome, '')
  const beforeReporting = JSON.stringify(['weeklyRecords', 'events', 'notifications'].map(collection => f.store.list(collection)))
  for (const patch of [{ status: 'done' }, { actualOutcome: '' }, { blocker: '' }, { submitted: true }]) {
    assert.throws(() => f.domain.updateWeeklyRecord(f.member, updatedWeek.id, { version: updatedWeek.version, ...patch }), { status: 400, code: 'WORK_VALIDATION_FAILED' })
    assert.equal(JSON.stringify(['weeklyRecords', 'events', 'notifications'].map(collection => f.store.list(collection))), beforeReporting)
  }
  const confirmedWeek = f.domain.updateWeeklyRecord(f.member, updatedWeek.id, { version: updatedWeek.version, status: 'done', actualOutcome: '补充核对：历史阶段成果已验收' })
  assert.equal(confirmedWeek.actualOutcome, '补充核对：历史阶段成果已验收')
  assert.deepEqual(confirmedWeek.importSource, imported.weekly.importSource)
