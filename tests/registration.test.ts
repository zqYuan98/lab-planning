...
test('self-registration requires initialization, eight characters, same-origin JSON; never grants a session or privileges', async () => {
  ...
  try {
    ...
    await f.request('/workspace', undefined, '', 'GET', 401)
...
test('manager review lifecycle is versioned; ordinary members, account PATCH and review of approved accounts cannot bypass it', async () => {
  ...
  try {
    ...
    const directory = await f.request('/workspace/directory/accounts?purpose=assignment', undefined, member.cookie)
    assert.equal(directory.data.items.some((user: User) => user.id === pending.id), false)
    assert.equal(buildReportSnapshot(f.store, 'monthly', '2026-09').users.some(user => user.id === pending.id), false)
    const admin = await f.request('/workspace/registration-requests?status=all', undefined, lead.cookie)
    assert.equal(admin.data.items.find((user: User) => user.id === pending.id).registrationStatus, 'pending')
    ...
    await f.request('/workspace', undefined, login.cookie)
...
test('unapproved status blocks forged active sessions and assignments; failed audits roll back registration and review', async () => {
  ...
  try {
    ...
    await f.request('/workspace', undefined, `lab_session=${token}`, 'GET', 401)
