...
test('API validation and server failures share generated response/log identifiers without logging secrets', async t => {
  ...
  const unauthenticated = await fetch(`${origin}/api/workspace`, { headers: { 'X-Request-Id': 'attacker-controlled' } })
  ...
  const original = store.workspaceCount.bind(store), originalError = console.error, entries: string[] = []
  store.workspaceCount = () => { throw new Error('private-message\nsecret-auth-value\n    at private-token (fake.ts:1:1)') }
  console.error = (...values: unknown[]) => { entries.push(values.map(String).join(' ')) }
  try {
    const response = await fetch(`${origin}/api/workspace?token=private-token`, { headers: { cookie } })
  ...
  } finally { store.workspaceCount = original; console.error = originalError }
})
