...
test('capture replays stable IDs after edits and rejects payload changes without duplicating tasks', t => {
  ...
  const edited = f.domain.updateTask(f.member, first[0].id, { version: first[0].version, title: '已澄清的试点范围', currentProgress: '已交流' })
...
test('capture rejects owner and origin spoofing including on replay for managers and members', t => {
  ...
  assert.throws(() => f.capture(), { status: 400 })
})
...
test('task metadata edits retain omitted fields, enforce versions and permissions, and support explicit unknown dates', t => {
  ...
  let edited = f.domain.updateTask(f.member, task.id, { version: task.version, ...metadata, dueDate: '2026-10-10', nextAction: '安排评审' })
  ...
  edited = f.domain.updateTask(f.member, task.id, { version: edited.version, dueDate: '', assignedOn: '' })
...
test('completion requires explanation and clears feedback, while a completed week keeps the task active', t => {
  ...
  task = f.domain.updateTask(f.member, task.id, { version: task.version, status: 'blocked', blockerReason: '等待数据', waitingForFeedback: true })
...
test('register metadata does not bypass assigned task deadline approval or collaboration blocker checks', t => {
  ...
  assert.throws(() => f.domain.updateTask(f.member, task.id, { version: task.version, workSource: 'leader', dueDate: '' }), { status: 409 })
...
test('legacy API retains its completion behavior while editing a legacy task through register requires explanations', t => {
  const f = fixture(t)
  ...
  const completed = f.domain.updateTask(f.member, legacy.id, { version: legacy.version, status: 'done' })
  assert.equal(completed.status, 'done')
  assert.equal(completed.waitingForFeedback, undefined)
  assert.equal(f.domain.updateTask(f.member, completed.id, { version: completed.version, status: 'blocked' }).status, 'blocked')
