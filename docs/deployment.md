...
恢复时停止应用，保留现有库及WAL副本。将已校验备份放到新的数据目录/命名卷，确保容器 `node` 用户有读写权限，通过 `DATA_VOLUME_NAME` 和 `DATABASE_PATH` 指向恢复副本。**启动应用前必须更新恢复库的操作环境**，防止恢复点之后的旧浏览器请求重复建立承接目标。确认 Compose 指向恢复卷后执行（路径按实际恢复库调整）：

```sh
docker compose run --rm --no-deps app npm run reset-operation-context -- /app/data/lab-planning.sqlite
```

裸机在应用停止时执行 `npm run reset-operation-context -- <恢复副本路径>`。该命令只更新内部操作环境，不修改计划、任务或报告；脚本要求数据库文件已存在。随后先用独立端口核验登录、项目、计划与报告，再切换正式入口。克隆和回退整库执行同一步，普通重启或只升级应用不轮换。不要覆盖唯一的现有库；不要使用会删除数据卷的清理命令。
