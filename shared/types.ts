...
export interface Task extends Entity { title: string; monthlyPlanId: string | null; ownerId: string; description: string; dueDate: string; status: 'todo' | 'doing' | 'blocked' | 'done'; isTemporary: boolean; temporaryReason: string; importSource?: ImportProvenance; workOrigin?: WorkOrigin }
export type WeeklyStatus = 'planned' | 'doing' | 'blocked' | 'done' | 'not_done'
export interface WeeklyRecord extends Entity { taskId: string; monthlyPlanId: string | null; ownerId: string; weekStart: string; commitment: string; actualOutcome: string; evidenceUrl: string; blocker: string; nextAction: string; status: WeeklyStatus; submitted: boolean; importSource?: ImportProvenance; workOrigin?: WorkOrigin }
