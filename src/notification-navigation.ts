...
export function entryLocation(location: { pathname: string; search: string }): { page: PageId; intent?: NavigationIntent } {
  ...
  if (location.pathname === '/work' && ['monthly', 'weekly', 'collaboration', 'reports', 'work-register', 'feedback'].includes(params.get('view') || '')) {
    ...
    if (['task', 'followup', 'digest', 'deadlineRequest'].includes(targetType || '')) intent.targetType = targetType as NavigationIntent['targetType']
...
export function navigationUrl(page: PageId, intent?: NavigationIntent): string {
  ...
  if (!['monthly', 'weekly', 'collaboration', 'reports', 'work-register', 'feedback'].includes(page)) return '/'
  const params = new URLSearchParams({ view: page })
  for (const key of ['id', 'ownerId', 'month', 'weekStart', 'cycleWeek', 'kind', 'targetType'] as const) if (intent?.[key]) params.set(key, intent[key]!)
...
export function notificationNavigation(target: NotificationTarget): { page: PageId; intent: NavigationIntent } {
  ...
  if (['followup', 'digest', 'deadlineRequest'].includes(target.type)) return { page: 'collaboration', intent: { id: target.id, targetType: target.type as NavigationIntent['targetType'] } }
