...
import { Badge, Empty, Field, Form, PageHeader, dateTime, nameOf, type PageProps } from '../ui'
...
export default function NotificationSettings({ data, notify, refresh }: PageProps) {
  ...
  const load = useCallback(async () => { setView(await api<NotificationSettingsView>('/notification-settings')) }, [])
  useEffect(() => { void load().catch(error => setError(error.message)) }, [load])
  async function retry(id: string) {
    if (busy) return
    setBusy(true); setError('')
    try { await api(`/notification-deliveries/${encodeURIComponent(id)}/retry`, json({})); await load(); notify('已重新排队，将在发送前再次核对权限与身份') }
  ...
  async function remind(id: string) {
    if (busy) return
    setBusy(true); setError('')
    try { await api(`/notifications/${encodeURIComponent(id)}/remind`, json({})); await load(); notify('提醒已记录，同一事项、同一成员每天最多一次') }
    finally { setBusy(false) }
  }
  return <div className="notifications-page">
    ...
    {!view ? <p role="status">正在读取通知设置…</p> : <>
      ...
      <section className="notification-settings-card"><div className="notification-section-heading"><Bell size={19} /><h2>发送范围与时段</h2></div>
        <Form key={`${view.settings.id}-${view.settings.version}`} submitLabel="保存通知设置" onSubmit={async event => {
          ...
          <div className="field"><span>试点成员</span><div className="notification-pilots">{data.users.filter(canUseAccount).map(user => {
            const bound = view.bindings.some(binding => binding.userId === user.id && binding.bound)
            return <label className="checkbox-label" key={user.id}><input type="checkbox" name="pilotUserIds" value={user.id} defaultChecked={view.settings.pilotUserIds.includes(user.id)} /><span>{user.name}<small>{user.position || (user.role === 'manager' ? '管理员' : '成员')}</small></span><Badge tone={bound ? 'green' : 'amber'}>{bound ? '已绑定' : '未绑定'}</Badge></label>
          })}</div></div>
      ...
      <section className="notification-settings-card"><div className="notification-section-heading"><h2>最近投递记录</h2><span>{view.deliveries.length} 条</span></div>
        ...
        {view.deliveries.length ? <div className="notification-deliveries">{view.deliveries.map(delivery => <article key={delivery.id}>
          ...
          <p>{nameOf(data, delivery.recipientId)} · 已尝试 {delivery.attempts} 次 · {dateTime(delivery.updatedAt)}</p>
          ...
          <div className="notification-status-actions">
            ...
            {delivery.status === 'failed' && <button type="button" className="button secondary" disabled={busy} onClick={() => void retry(delivery.id)}>重试明确失败的投递</button>}
            {delivery.canAcknowledge && !delivery.acknowledgedAt && <button type="button" className="button secondary" disabled={busy} onClick={() => setPreview({ notificationId: delivery.notificationId, reminder: true })}>提醒本人确认</button>}
          </div>
    ...
    <NotificationDiagnostics users={data.users} />
