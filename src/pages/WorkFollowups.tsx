...
import type { DigestItem, NotificationDigest, WorkRisk } from '../../shared/collaboration-notifications'
...
export default function WorkFollowups(props: PageProps & { navigate: Navigate }) {
  ...
  async function openDigest(id: string) {
    const sequence = ++digestSequence.current
    try { const row = await api<NotificationDigest & { items: DigestItem[] }>(`/digests/${encodeURIComponent(id)}`); if (sequence === digestSequence.current) setDigest(row) }
    catch (e) { if (sequence === digestSequence.current) setError(e instanceof Error ? e.message : '摘要读取失败') }
  }
  const load = useCallback(async () => { setView(await api<Dashboard>('/collaboration')) }, [])
  useEffect(() => { void load().catch(e => setError(e.message)) }, [load])
  useEffect(() => {
    if (!intent?.id) return
    let live = true
    if (intent.targetType === 'digest') void openDigest(intent.id)
    else void api<{ taskId: string }>(`/collaboration/resolve/${intent.targetType || 'task'}/${encodeURIComponent(intent.id)}`).then(row => { if (live) setTaskId(row.taskId) }).catch(e => { if (live) setError(e.message) })
  ...
  return <div className="collaboration-page">
    ...
    {batch && <Modal wide title="预览批量催办" onClose={() => { previewSequence.current++; setBatch(false) }}><Form submitLabel="生成预览" onSubmit={async e => { const sequence = ++previewSequence.current; const f = new FormData(e.currentTarget), next = { taskIds: selected, requirement: String(f.get('requirement')), ...(f.get('dueAt') ? { dueAt: new Date(`${f.get('dueAt')}:00+08:00`).toISOString() } : {}), enroll: f.has('enroll') }; const result = await api<BatchPreview>('/followups/preview', json(next)); if (sequence !== previewSequence.current) return; setPayload({ ...next, dueAt: result.dueAt }); setPreview(result) }}><div onChange={() => { previewSequence.current++; setPreview(null); setPayload(null) }}><Field label="希望负责人更新什么"><textarea name="requirement" required rows={3} /></Field><Field label="回应期限（北京时间，留空则下一个工作日 17:00）"><input type="datetime-local" name="dueAt" /></Field><label className="checkbox-label"><input name="enroll" type="checkbox" />将所选未跟踪任务明确纳入督办</label></div></Form>
      {preview && payload && <div className="collaboration-preview"><p>本次更新要求：{String(payload.requirement)}</p><p>将按 {preview.recipients.length} 位接收人合并通知，回应期限 {dateTime(preview.dueAt)}。</p>{preview.recipients.map(recipient => <article key={recipient.recipientId}><h3>{recipient.recipientName}</h3><p>{recipient.externalQuotaAvailable ? '按当前外发设置投递' : '今日提醒已达额度，保留站内事项'}</p>{recipient.items.map(item => <p key={item.taskId}>{item.title} · 原截止 {item.taskDueDate}{item.existingRequest ? ' · 已有待回应催办，将复用' : ''}{item.enrollRequired ? ' · 需纳入督办' : ''}</p>)}</article>)}<Form submitLabel="确认发起催办" onSubmit={async () => { const body = { ...payload, previewToken: preview.previewToken }; attempt.current = assignmentAttempt(attempt.current, body); await api('/followups/batch', json({ ...body, requestId: attempt.current.requestId })); attempt.current = null; setBatch(false); setSelected([]); await load(); notify('催办已保存，按接收人合并通知') }}><p className="form-hint">不会改写任务截止时间。已有催办不会重复新建；收到回应后会结束该催办。</p></Form></div>}
    </Modal>}
    {digest && <Modal wide title="工作摘要详情" onClose={() => { digestSequence.current++; setDigest(null) }}><p>{dateTime(digest.generatedAt)} · 内容按当前权限展示</p>{manager && digest.statistics?.length ? <p>周期统计（含本期已回告事实）：{digest.statistics.map(item => `${item.label} ${item.value}`).join("；")}</p> : null}{digest.items.length ? digest.items.map(item => <article className="collaboration-history" key={item.id}><h3>{item.title}</h3>{item.lines.map((line, index) => <p key={index}>{line}</p>)}<button className="button secondary" onClick={() => { const destination = notificationNavigation(item.target); setDigest(null); if (item.taskId) setTaskId(item.taskId); else navigate(destination.page, destination.intent) }}>查看当前事项</button></article>) : <p>摘要所含事项已删除、转交或当前无权访问。</p>}</Modal>}
  </div>
}
