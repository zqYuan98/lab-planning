...
export default function WorkRegisterReport({ snapshot, onClose }: { snapshot: Snapshot; onClose: () => void }) {
  ...
  return createPortal(<div className="wr-report-portal"><Modal wide title="工作清单 · 汇报预览" onClose={onClose}>
    <div className="wr-report-controls"><p>非正式报告。已固定本次预览内容，导出和打印均以此为准。</p><div><button className="button secondary" onClick={() => window.print()}><Printer size={16} />打印 / 存为 PDF</button><button className="button primary" onClick={() => download(snapshot)}><Download size={16} />导出 CSV</button></div></div>
    <section className="wr-report" aria-label="本人工作清单汇报">
      ...
      {snapshot.rows.length ? <div className="wr-report-items">{snapshot.rows.map((row, index) => <article key={`${row.itemType}:${row.id}`} className="wr-report-item">
        ...
        <dl className="wr-report-facts"><div><dt>来源 / 交办人</dt><dd>{row.source} · {row.assignedBy}</dd></div><div><dt>交办日期</dt><dd>{row.assignedOn}</dd></div><div><dt>截止日期</dt><dd>{row.dueDate}</dd></div><div><dt>优先级</dt><dd>{row.priority}</dd></div><div><dt>预计剩余投入</dt><dd>{row.remainingEffortDays}</dd></div><div><dt>投入备注</dt><dd>{row.estimatedEffort}</dd></div><div><dt>排期 / 反馈</dt><dd>{row.schedule} · {row.waitingForFeedback}</dd></div></dl>
