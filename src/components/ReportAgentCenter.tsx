...
export default function ReportAgentCenter({ data, refresh, notify, onOpenReport }: {
  data: Bootstrap; refresh: () => Promise<void>; notify: (message: string) => void; onOpenReport: (report: Report) => void
}) {
  ...
  const [period, setPeriod] = useState(weekMonday(shanghaiToday())), [templateId, setTemplateId] = useState('')
  ...
  useEffect(() => {
    ...
    api<ReportAgentBootstrap>('/report-agent', { signal: controller.signal }).then(value => { setBootstrap(value); setError('') }).catch(failure => { if (!controller.signal.aborted) setError(failure instanceof Error ? failure.message : '无法读取周报智能体') })
  ...
  const resolvedTemplateId = templateId || activeTemplates.find(template => period && template.effectiveWeek <= weekMonday(period))?.id || ''
  if (data.user.role !== 'manager') return <Empty title="周报智能体面向部门管理者" />
  return <div className="report-agent">
    <div className="agent-intro"><div><h2>周报模板与生成</h2><p>沿用公司 Word 模板，冻结本期事实，逐项核对后定稿。</p></div><Badge tone="neutral">周报</Badge></div>
    <ol className="agent-steps" aria-label="周报工作流程"><li>上传模板与范例</li><li>确认映射与写法</li><li>试填并启用</li><li>生成、审阅与定稿</li></ol>
    <nav className="report-tabs" aria-label="周报智能体功能"><button aria-pressed={tab === 'templates'} onClick={() => switchTab('templates')}>模板与范例</button><button aria-pressed={tab === 'generate'} onClick={() => switchTab('generate')}>生成与任务</button><button aria-pressed={tab === 'schedule'} onClick={() => switchTab('schedule')}>每周自动草稿</button></nav>
    ...
    {!bootstrap ? <p role="status">正在读取模板和任务…</p> : <>
      {tab === 'templates' && <div className="agent-template-layout"><aside className="agent-template-list" aria-label="周报模板版本"><UploadTemplate accountId={data.user.id} notify={notify} onCreated={template => { setSelected(template); setTemplateEpoch(value => value + 1); void action(load) }} onUploaded={load} assets={bootstrap.assets} />
        <h3>已保存模板</h3>{templates.length ? templates.map(template => <button key={template.id} className={`agent-template-choice ${selected?.id === template.id ? 'is-selected' : ''}`} onClick={() => { if (allowDraftLeave()) { setSelected(template); setTemplateEpoch(value => value + 1) } }}><strong>{template.name}</strong><span>{template.status === 'active' ? '已启用' : template.status === 'draft' ? '待确认' : '已停用'} · 版本 {template.version}</span><small>{template.effectiveWeek} 起适用</small></button>) : <p className="agent-note">上传一份公司模板即可开始。</p>}</aside>
        <div className="agent-template-detail">{selected ? <><div className="agent-toolbar"><p className="agent-note">学习任务完成后，可重新读取模板查看候选规则。当前编辑不会被任务进度覆盖。</p><button type="button" className="button secondary" disabled={busy} onClick={() => { if (allowDraftLeave()) void action(async () => { const latest = await api<ReportAgentBootstrap>('/report-agent'); setBootstrap(latest); const template = latest.templates.find(item => item.id === selected.id); if (template) { setSelected(template); setTemplateEpoch(value => value + 1) } }) }}><RefreshCw size={15} />重新读取</button></div><ReportAgentTemplate key={`${selected.id}:${templateEpoch}`} initial={selected} assets={bootstrap.assets} accountId={data.user.id} aiConfigured={bootstrap.aiConfigured} notify={notify} onSaved={rememberTemplate} onJob={rememberJob} /></> : <Empty title="建立可复用的周报模板" description="上传原 Word 和认可的历史范例后，逐表查看原文、确认填写来源，试填核对通过后再启用。" />}</div></div>}
      {tab === 'generate' && <section className="agent-generation"><h3>选择周次，生成待审阅草稿</h3><div className="agent-form-grid"><Field label="报告周次" hint="以周一至周日为一个报告周期。"><input type="date" value={period} onChange={event => setPeriod(event.target.value)} /></Field><Field label="已启用的 Word 模板"><select value={resolvedTemplateId} onChange={event => setTemplateId(event.target.value)}><option value="">请选择模板</option>{activeTemplates.map(template => <option key={template.id} value={template.id}>{template.name}（{template.effectiveWeek} 起）</option>)}</select></Field></div><p className="agent-note">本次选择：{period ? weekMonday(period) : '待选周次'} 起的一周。生成时读取当前数据并冻结；历史周次显示本次实际截至时间。</p><label className="agent-check"><input type="checkbox" checked={useAi} disabled={!bootstrap.aiConfigured || busy} onChange={event => setUseAi(event.target.checked)} />使用已配置的 AI 按已确认规则辅助写作</label>{!bootstrap.aiConfigured && <p className="agent-note">尚未配置 AI，仍可生成带来源的规则草稿并逐格编辑。</p>}<button className="button primary" disabled={busy || !resolvedTemplateId || !period} onClick={() => void action(async () => {
        const payload = { templateId: resolvedTemplateId, period: weekMonday(period), useAi }
      ...
      })}><Plus size={16} />生成周报草稿</button>
        <h3>周报档案</h3><div className="agent-report-list">{bootstrap.reports.length ? bootstrap.reports.map(report => <button key={report.id} className="agent-template-choice" onClick={() => void openReport(report.id)}><strong>{report.title}</strong><span>{report.period} · 第 {report.revision} 版 · {report.status === 'finalized' ? '已定稿' : '待审阅'}</span></button>) : <p className="agent-note">完成第一项生成任务后，在这里审阅报告。</p>}</div>
      </section>}
      {tab === 'schedule' && <ReportAgentScheduleForm key={bootstrap.schedule.id} initial={bootstrap.schedule} templates={activeTemplates} data={data} missedPeriods={bootstrap.missedPeriods} notify={notify} onSaved={value => setBootstrap(current => current ? { ...current, schedule: value } : current)} />}
      {(tab === 'generate' || bootstrap.jobs.some(job => agentJobPending(job.status) || job.status === 'failed')) && <section className="agent-jobs" aria-label="后台任务"><h3>生成与学习任务</h3>{bootstrap.jobs.slice(0, 12).map(job => <ReportAgentJobCard key={job.id} initial={job} onOpen={id => void openReport(id)} onComplete={() => { void load().catch(failure => setError(`任务进度已更新，但列表刷新失败：${failure instanceof Error ? failure.message : '请重新读取'}`)); void refresh().catch(() => {}) }} />)}</section>}
    </>}
  </div>
}
function UploadTemplate({ accountId, assets, onCreated, onUploaded, notify }: { accountId: string; assets: ReportAssetSummary[]; onCreated: (template: ReportTemplate) => void; onUploaded: () => Promise<void>; notify: (message: string) => void }) {
  const [file, setFile] = useState<File | null>(null), [examples, setExamples] = useState<File[]>([])
  const [uploadEpoch, setUploadEpoch] = useState(0)
  const [sourceAssetId, setSourceAssetId] = useState(''), [name, setName] = useState(''), [week, setWeek] = useState(weekMonday(shanghaiToday()))
  ...
  const recovery = useFormDraft(`${accountId}:report-agent-upload`, { name, sourceAssetId, week, selectedFile: file?.name || '', selectedExamples: examples.map(item => item.name) }, values => { if (typeof values.name === 'string') setName(values.name); if (typeof values.sourceAssetId === 'string') setSourceAssetId(values.sourceAssetId); if (typeof values.week === 'string') setWeek(values.week) }, busy)
  ...
  async function submit() {
    ...
    try {
      ...
      const template = await api<ReportTemplate>('/report-agent/templates', json({ name: name.trim() || file?.name.replace(/\.docx$/i, '') || '公司周报模板', sourceAssetId: source, exampleAssetIds: ids, effectiveWeek: weekMonday(week), requestId: requestId.current }))
