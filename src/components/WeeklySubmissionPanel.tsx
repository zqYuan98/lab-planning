  ...
  return (
    <section className="weekly-submission-panel" aria-label="周五提报">
      ...
      {currentView && view && <>
        ...
        {view.rule.planReviewEffectiveWeek && <p className="submission-explanation">周计划审核从 {view.rule.planReviewEffectiveWeek} 提报周期开始，覆盖 {advanceWeek(view.rule.planReviewEffectiveWeek, 7)} 起的下周计划。提交时效与审核结果分别记录，日常进展更新不重审。</p>}
      ...
      {mode === 'submit' && selected && <Modal title={`${kindLabel(selected.kind)} · 整份提交`} onClose={() => setMode('')} wide>
        ...
        {selected.kind === 'plan' && selected.planReviewRequired && <p className="submission-explanation">本次提交保存一份计划快照供管理员审核。提交时间单独记录；等待审核或退回修改不会抹去原提交时间。</p>}
        {recordPreview(selected)}
        <div className="submission-actions"><button className="button secondary" disabled={!data.users.some(user => user.id === selected.ownerId && canUseAccount(user))} onClick={() => selectWork(selected, undefined, true)}>添加该周任务</button><button className="button secondary" disabled={loading} onClick={() => void refreshSelected()}>刷新并重新核对</button></div>
      ...
      {mode === 'review' && selected?.latestSubmission && manager && <Modal title="审核下周计划" onClose={() => setMode('')} wide>
        ...
        <button className="text-button" disabled={loading} onClick={() => void refreshSelected('review')}>刷新待审版本</button>
        {selected.planReviewStatus === 'pending' ? <Form key={selected.latestSubmission.id} onCancel={() => setMode('')} submitLabel={reviewDecision === 'approved' ? '确认审核通过' : '退回修改'} onSubmit={async event => {
      ...
      {mode === 'roster' && view?.cycle && manager && <Modal title="核对应交名单" onClose={() => setMode('')}>
        ...
        <Form onCancel={() => setMode('')} onSubmit={async event => {
          ...
          {historicalRosterAccounts(data.users).map(user => <label className="check-field" key={user.id}><input name="rosterIds" type="checkbox" value={user.id} defaultChecked={view.cycle!.rosterIds.includes(user.id)} />{accountDisplayName(user)}</label>)}
