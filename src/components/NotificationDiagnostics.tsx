...
export default function NotificationDiagnostics({ users }: { users: User[] }) {
  ...
  useEffect(() => {
    ...
    void api<View>(`/notification-diagnostics?${params}`, { signal: controller.signal }).then(result => { if (current === sequence.current) setView(result) }).catch(error => {
  ...
  }, [status, recipient, kind, from, to, cursor, revision])
  function filter(setter: (value: string) => void, value: string) { setCursor(undefined); setView(undefined); setter(value) }
  async function retry(id: string) {
    if (busy) return
    setBusy(true); setError('')
    try { await api(`/notification-deliveries/${encodeURIComponent(id)}/retry`, json({})); setRevision(value => value + 1) }
  ...
  return <section className="notification-settings-card" aria-label="通知运行诊断">
    ...
    <div className="form-grid">
      ...
      <label className="field"><span>成员</span><select value={recipient} onChange={event => filter(setRecipient, event.target.value)}><option value="">全部成员</option>{users.map(user => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label>
    ...
    {view && <>
      ...
      <div className="notification-deliveries">{view.items.map(item => <article key={item.id}><div className="notification-delivery-title"><strong>{item.recipientName} · {deliveryLabels[item.status]}</strong><small>{item.kind}</small></div>
        ...
        {item.status === 'failed' && <button className="button secondary" disabled={busy} onClick={() => void retry(item.id)}>重试明确失败的投递</button>}
      </article>)}</div>
