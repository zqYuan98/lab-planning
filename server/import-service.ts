...
import type { ImportBatch, ImportBatchSummary, ImportRow } from '../shared/import-types.ts'
...
interface ImportLink extends Entity { batchId: string; rowId: string; result: { collection: string; id: string }; rowFingerprint: string }
...
export class ImportService {
  ...
  async upload(actor: User, input: Input): Promise<ImportBatch> {
    ...
    return this.store.transaction(() => {
      ...
      const batch = this.store.insert<ImportBatch>('importBatches', { ownerId: actor.id, sourceId, fileName, kind: parsed.kind, status: 'uploaded', sourceSheets: parsed.sheets?.map(s => ({ name: s.name, rowCount: s.rows.length })) ?? [], warnings: parsed.warnings, rows: [], mode: 'history' })
  ...
  private checked(actor: User, rows: ImportRow[]) { return rows.map(row => ({ ...row, issues: this.issues(actor, row, rows) })) }
  ...
  async analyze(actor: User, id: string, input: Input, progress?: (completed: number, total: number) => void, credentialGuard?: () => void): Promise<ImportBatch> {
    ...
    try {
      ...
      const checked = this.checked(actor, this.match(actor, rows))
      return this.store.transaction(() => {
        checkpoint()
        this.mutable(actor, id, before.version)
        const result = this.store.update<ImportBatch>('importBatches', id, before.version, { status: 'parsed', rows: checked, warnings: [...new Set(warnings)].slice(0, 100) })
  ...
  edit(actor: User, id: string, input: Input): ImportBatch {
    ...
    return this.store.transaction(() => {
      ...
      if (!['history', 'draft'].includes(String(mode))) throw new HttpError(400, '导入方式无效')
      ...
      const rows = input.rows.map((value, index) => {
        ...
        return this.normalizeRow(row, index, original)
      })
      const result = this.store.update<ImportBatch>('importBatches', id, before.version, { rows: this.checked(actor, rows), mode: mode as ImportBatch['mode'] })
  ...
  structured(actor: User, input: Input): ImportBatch {
    ...
    return this.store.transaction(() => {
      ...
      const batch = this.store.insert<ImportBatch>('importBatches', { ownerId: actor.id, sourceId, fileName: parsed.fileName, kind: 'text', status: 'parsed', sourceSheets: [], warnings: [], rows: this.checked(actor, rows), mode: input.mode === 'draft' ? 'draft' : 'history' })
  ...
  commit(actor: User, id: string, input: Input): ImportBatch {
    ...
    return this.store.transaction(() => {
      ...
      const rows = this.checked(actor, batch.rows), selected = rows.filter(r => r.selected)
      ...
      // Monthly drafts are created first so weekly rows can refer to them atomically.
      for (const row of [...selected.filter(r => r.kind === 'monthly'), ...selected.filter(r => r.kind === 'weekly')]) {
        const linkId = hash(`${batch.sourceId}:${row.id}:${batch.mode}`)
        const previous = this.store.get<ImportLink>('importLinks', linkId)
        if (previous) {
        ...
        if (batch.mode === 'history') {
          ...
          const plan = this.domain.createPlan(actor, { month: row.month, title: row.title, projectId: row.projectId || undefined, category: row.category, ownerId: row.ownerId, expectedOutcome: row.expectedOutcome, acceptanceCriteria: row.acceptanceCriteria, dueDate: row.dueDate })
        ...
        if (batch.mode === 'draft') {
          // A new draft cannot claim a historical publication/acceptance. Keep original
          // results as queryable source facts alongside the new operational record.
          this.store.insert<HistoricalRecord>('historicalRecords', { importedBy: actor.id, batchId: id, sourceId: batch.sourceId, row: { ...row } })
        }
        written++
        this.store.insert<ImportLink>('importLinks', { id: linkId, batchId: id, rowId: row.id, result: row.result!, rowFingerprint: rowFingerprint(row) })
      }
      const result = this.store.update<ImportBatch>('importBatches', id, batch.version, { rows, status: 'committed', committedAt: new Date().toISOString(), committedCount: written, skippedCount: skipped })
      this.audit(actor, id, 'commit', { mode: batch.mode, count: written, skipped, records: selected.map(r => r.result) })
      return result
    })
  }
}
