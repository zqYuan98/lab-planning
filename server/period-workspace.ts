...
export class PeriodWorkspaceService {
  ...
  private planVisibility(actor: User, plans: MonthlyPlan[]) {
    ...
    if (isManager(actor)) return plans
  ...
  private task(actor: User, id: string): Task | undefined {
    ...
    if (current && (isManager(actor) || current.ownerId === actor.id)) return current
    const snapshots = this.store.entityEvents('task', id).flatMap(event => [event.before, event.after]).filter((row): row is Task => !!row && (row as Task).id === id && (isManager(actor) || (row as Task).ownerId === actor.id)).sort((a, b) => b.version - a.version)
  ...
  references(actor: User, records: WeeklyRecord[] = [], plans: MonthlyPlan[] = [], tasks: Task[] = []): PeriodReferences {
    ...
    for (const id of taskIds) if (!taskMap.has(id)) { const current = currentTasks.get(id), task = current && (isManager(actor) || current.ownerId === actor.id) ? isActiveTask(current) ? current : undefined : this.task(actor, id); if (task) taskMap.set(task.id, task) }
  ...
  weekly(actor: User, input: Query): WeeklyWorkspace {
    return this.store.readTransaction(() => {
      ...
      const id = queryText(input.id), direct = id ? this.store.get<WeeklyRecord>('weeklyRecords', id) : undefined, record = direct && isActiveWeeklyRecord(direct) && direct.weekStart === week && (isManager(actor) || direct.ownerId === actor.id) ? direct : id ? rows.find(row => row.taskId === id) : undefined, directTask = id && !record ? this.store.get<Task>('tasks', id) : undefined, task = record ? this.task(actor, record.taskId) : directTask && isActiveTask(directTask) && (isManager(actor) || directTask.ownerId === actor.id) ? directTask : undefined
  ...
  record(actor: User, id: string) { const ctx = pageContext(this.store, actor), record = this.store.get<WeeklyRecord>('weeklyRecords', id); if (!record || !isActiveWeeklyRecord(record) || !isManager(ctx.actor) && record.ownerId !== ctx.actor.id || !this.task(ctx.actor, record.taskId)) throw new HttpError(404, '周记录不存在或当前不可访问'); return { record, references: this.references(ctx.actor, [record]) } }
  candidates(actor: User, input: Query): PeriodCandidates {
    return this.store.readTransaction(() => {
      ...
      if (!isManager(actor) && owner !== actor.id || ['publish', 'merge', 'relink'].includes(purpose) && !isManager(actor)) throw new HttpError(403, '无权读取该候选范围')
  ...
  submissionReferences(actor: User, input: Query) {
    ...
    let rows = this.store.selectJson<WeeklyRecord>("SELECT data FROM entities WHERE collection='weeklyRecords' AND json_extract(data,'$.weekStart') IN (?,?) AND (?=1 OR json_extract(data,'$.ownerId')=?) ORDER BY rowid", [week, end.toISOString().slice(0, 10), Number(isManager(ctx.actor)), ctx.actor.id])
    const events = rows.length ? this.store.selectJson<AuditEvent>("SELECT data FROM entities WHERE collection='events' AND json_extract(data,'$.entityType')='weeklyRecord' AND json_extract(data,'$.entityId') IN (SELECT value FROM json_each(?)) ORDER BY rowid", [JSON.stringify(rows.map(row => row.id))]) : []
