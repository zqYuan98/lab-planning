...
export class PeriodWorkspaceService {
  ...
  private planRows(actor: User, month?: string): MonthlyPlan[] {
    const plans = month ? this.store.selectJson<MonthlyPlan>("SELECT data FROM entities p WHERE collection='plans' AND (json_extract(data,'$.month')=? OR id IN (SELECT json_extract(s.value,'$.id') FROM entities e,json_each(json_array(json_extract(e.data,'$.before'),json_extract(e.data,'$.after'))) s WHERE e.collection='events' AND json_extract(e.data,'$.entityType')='plan' AND json_extract(s.value,'$.month')=? AND (json_extract(s.value,'$.ownerId')=? OR EXISTS(SELECT 1 FROM json_each(s.value,'$.collaboratorIds') c WHERE c.value=?))) OR id IN (SELECT json_extract(s.value,'$.id') FROM entities e,json_each(e.data,'$.plans') s WHERE e.collection='publications' AND json_extract(s.value,'$.month')=? AND (json_extract(s.value,'$.ownerId')=? OR EXISTS(SELECT 1 FROM json_each(s.value,'$.collaboratorIds') c WHERE c.value=?)))) ORDER BY rowid", [month, month, actor.id, actor.id, month, actor.id, actor.id]) : this.store.selectJson<MonthlyPlan>("SELECT data FROM entities WHERE collection='plans' AND (?=1 OR json_extract(data,'$.ownerId')=? OR EXISTS(SELECT 1 FROM json_each(data,'$.collaboratorIds') c WHERE c.value=?)) ORDER BY rowid", [Number(actor.role === 'manager'), actor.id, actor.id])
