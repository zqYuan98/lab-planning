...
import { normalizeReportPeriod } from './reports.ts'
...
export const reportTemplateTransferSchema = z.object({ ...entity, name: z.string().min(1).max(200), type: z.enum(['weekly', 'monthly']).default('weekly'), status: z.enum(['draft', 'active', 'archived']),
  ...
  effectiveWeek: z.string().max(10), activatedAt: timestamp.nullable(), createdBy: id,
}).strict()
...
export const reportAgentPayloadSchema = z.object({ schemaVersion: z.enum(['weekly-v1', 'monthly-v1']), capturedAt: timestamp, snapshotHash: hash, template: reportTemplateTransferSchema, templateHash: hash,
...
export function reportAgentTransferIssues(rows: BusinessCollections, available: Record<TransferCollection, Map<string, Entity>>, issue: (message: string) => void) {
  ...
  const inspectTemplate = (template: ReportTemplate, label: string) => {
    try { if (normalizeReportPeriod(template.type || 'weekly', template.effectiveWeek) !== template.effectiveWeek) issue(`${label}：模板适用周期不规范`) } catch { issue(`${label}：模板类型与适用周期不一致`) }
  ...
  for (const report of rows.reports.filter((row: Report) => row.agent)) {
    ...
    if ((agent.template.type || 'weekly') !== report.type || agent.schemaVersion !== (report.type === 'monthly' ? 'monthly-v1' : 'weekly-v1')) issue(`reports/${report.id}：报告类型、模板类型与事实版本不一致`)
    try { if (normalizeReportPeriod(report.type, report.period) !== report.period || agent.template.effectiveWeek > report.period) issue(`reports/${report.id}：报告周期与模板适用期不一致`) } catch { issue(`reports/${report.id}：报告类型与周期不一致`) }
